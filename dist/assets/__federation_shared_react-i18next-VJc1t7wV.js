import { importShared } from './__federation_fn_import-C7C6N0XN.js';
...
const {Fragment,isValidElement,cloneElement,createElement: createElement$3,Children} = await importShared('react');
      ...
      if (node.type === 'tag') {
        ...
        if (isString(child)) {
          ...
          if (isKnownComponent) {
            ...
            if (node.voidElement) {
              mem.push(createElement$3(node.name, {
                key: `${node.name}-${i}`
              }));
            } else {
              const inner = mapAST(reactNodes, node.children, rootReactNode);
        ...
        if (wrapTextNodes) {
          mem.push(createElement$3(wrapTextNodes, {
            key: `${node.name}-${i}`
          }, content));
        } else {
          mem.push(content);
...
const fixComponentProps = (component, index, translation) => {
  ...
  function Componentized() {
    return createElement$3(Fragment, null, comp);
  }
  return createElement$3(Componentized, {
    key: componentKey
  });
};
...
function Trans$1({
  ...
  return useAsParent ? createElement$3(useAsParent, additionalProps, content) : content;
}
...
const {useState,useEffect,useContext: useContext$1,useRef,useCallback} = await importShared('react');
...
const {createElement: createElement$2,forwardRef:forwardRefReact} = await importShared('react');
const withTranslation = (ns, options = {}) => function Extend(WrappedComponent) {
  function I18nextWithTranslation({
    ...
    return createElement$2(WrappedComponent, passDownProps);
  }
  I18nextWithTranslation.displayName = `withI18nextTranslation(${getDisplayName(WrappedComponent)})`;
  I18nextWithTranslation.WrappedComponent = WrappedComponent;
  const forwardRef = (props, ref) => createElement$2(I18nextWithTranslation, Object.assign({}, props, {
    forwardedRef: ref
  }));
...
const {createElement: createElement$1,useMemo} = await importShared('react');
